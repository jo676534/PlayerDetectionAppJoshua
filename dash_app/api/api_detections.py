...
def get_detection_data(game_id, start, end):
    conn = pg2.connect(
        database=database, user=user, host=host, password=password
    )  # ctrl + d
    cur = conn.cursor()

    cur.execute(
        f"""SELECT * FROM detections WHERE game_id={game_id} AND frame>={start} AND frame<={end}"""
    )  # fstrings confirmed to work
    data = cur.fetchall()
...
def save_track(game_id, detections_df, frame, track_id, player_id):
    ...
    for index, det in detections_df.iterrows():
        cur.execute(
            f"""INSERT INTO detections (game_id, frame, x0, y0, x1, y1, track_id, player_id) VALUES ({game_id}, {frame}, {det['x0']}, {det['y0']}, {det['x1']}, {det['y1']}, {track_id}, {player_id})"""
        )
        frame += 1
...
def unique_track_id(game_id):
    ...
    cur.execute(
        f"""SELECT MAX(track_id) FROM detections WHERE game_id={game_id}"""
    )  # fstrings confirmed to work
    data = cur.fetchall()
...
def delete_detection(game_id, frame, track_id):
    ...
    cur.execute(
        f"""DELETE FROM detections WHERE game_id={game_id} AND frame={frame} AND track_id={track_id}"""
    )
...
def delete_detection_section(game_id, start_frame, final_frame, track_id):
    ...
    cur.execute(
        f"""DELETE FROM detections WHERE game_id={game_id} AND track_id={track_id} AND frame >= {start_frame} AND frame <= {final_frame}"""
    )
...
def delete_detection_list(game_id, track_id, arr):
    ...
    st = set(arr)
    cur.execute(
        f"""DELETE FROM detections WHERE game_id={game_id} AND track_id={track_id} AND frame=ANY('{st}')"""
    )
...
def add_detection(game_id, frame, x0, y0, x1, y1, track_id, player_id, initials):
    ...
    cur.execute(
        """INSERT INTO detections (game_id, frame, x0, y0, x1, y1, track_id, player_id, initials) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
...
def get_partial_frame_detections(game_id, start_frame, final_frame):
    ...
    while frame <= final_frame:
        ...
        cur_temp.execute(
            f"""SELECT * FROM detections WHERE game_id={game_id} AND frame={frame}"""
        )
...
def assign_track(game_id, player_id, track_id):
    ...
    cur = conn.cursor()
    cur.execute(
        """UPDATE detections SET player_id=%s, initials=%s WHERE track_id=%s AND game_id=%s""",
        (player_id, initials, track_id, game_id),
    )
...
def unassign_track(game_id, track_id):
    ...
    cur = conn.cursor()
    cur.execute(
        f"""UPDATE detections SET player_id=-1 WHERE track_id={track_id} AND game_id={game_id}"""
    )
...
def delete_track(game_id, track_id):
    ...
    cur = conn.cursor()
    cur.execute(
        f"""DELETE FROM detections WHERE track_id={track_id} and game_id={game_id}"""
    )
    conn.commit()
    cur.close()
    conn.close()
...
def get_player_frames(game_id, player_id):
    ...
    cur.execute(
        """SELECT frame FROM detections WHERE game_id=%s AND player_id=%s""",
        (game_id, player_id),
    )
    data = cur.fetchall()
...
def get_track_frames(game_id, track_id):
    ...
    cur.execute(
        f"""SELECT frame FROM detections WHERE game_id={game_id} AND track_id={track_id}"""
    )
    data = cur.fetchall()
