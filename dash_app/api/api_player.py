...
def get_player(game_id, player_id):
    ...
    cur.execute(
        f"""SELECT * FROM roster where game_id={game_id} and player_id={player_id}"""
    )
    players_data = cur.fetchall()
...
def assign_track(game_id, track_id, player_id):
    ...
    # Query/Commit Here
    cur.execute(
        f"""UPDATE detections SET player_id={player_id} WHERE game_id={game_id} AND track_id ={track_id}"""
    )
...
def get_player_detections(game_id, player_id):
    ...
    cur.execute(
        f"""SELECT * FROM detections WHERE game_id={game_id} AND player_id={player_id}"""
    )
    data = cur.fetchall()
