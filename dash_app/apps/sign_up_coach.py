...
password_description_component = html.Div(
    [
        dbc.FormText(
            id="length_description_id",
            children="* Your Password must include at least 8 characters",
        ),
        dbc.FormText(
            id="uppercase_description_id",
            children="* At least one uppercase letter is needed",
        ),
        dbc.FormText(
            id="lowercase_description_id",
            children="* At least one lowercase letter is needed",
        ),
        dbc.FormText(
            id="number_description_id", children="* At least one number [0-9] is needed"
        ),
        dbc.FormText(
            id="special_char_description_id",
...
password_input_component = html.Div(
    [
        ...
        dbc.FormText(
            id="password_validation_text", children=password_description_component
        ),
    ]
)
...
confirmation_code_module = dbc.Modal(
    [
        ...
        dbc.ModalBody(
            id="modal_confirm_account_body_id", children=component_inside_modal
        ),
        dbc.ModalFooter(
            html.H5(
                "You will be redirected to the login page once successfully confirmed account."
            )
        ),
    ],
    ...
    [
        ...
        dbc.CardFooter(
            [
                ...
                error_signing_up,
                html.Div(
                    id="sign_up_button_container_id",
                    children=sign_up_button_component,
                    style={"textAlign": "center"},
                ),
            ]
        ),
...
layout = html.Div(
    children=card,
    style={"padding-top": "50px", "padding-left": "300px", "padding-right": "300px"},
)


@app.callback(
    Output("validates_code_button_id", "disabled"),
...
def enable_sign_up_button(
    ...
    if (
        ...
        and user_last_name != None
        and email_validation_text == None
        and user_email != None
        and password_validation_text == "* Passwords MATCH"
...
def sign_up_user_attempt(
    ...
    if n_clicks is None:
        ...
        if n_clicks:
            ...
            res = aws_cognito.attempt_to_sign_up_coach_user(
                first_name, last_name, user_email, user_password, phone_number
            )
...
def validate_email(email):
    if email is None:
        ...
        if re.fullmatch(regex, string_email):
            ...
            return "Invalid Email!", {"color": "red"}
...
def return_password_strength(password):
    ...
    while strong_password_list["points"] != 5 and counter < len(str(password)):
        ...
        if (
            any(char in str(password) for char in special_char_symbols)
            and strong_password_list["isspecialchar"] == False
        ):
            strong_password_list["points"] += 1
...
def validate_reenter_password(
    reenter_user_password, user_password, reenter_user_password_boolean, error_alert_id
):

    if (
        reenter_user_password is None and reenter_user_password_boolean == False
    ) or reenter_user_password_boolean == True:
        raise PreventUpdate
    else:
        ...
        if str(user_password) != str(reenter_user_password):
            ...
            return "* Passwords MATCH", {"color": "green"}


@app.callback(Output("phone_number", "value"), Input("phone_number", "value"))
def validate_phone_number(phone_number):
    ...
    if len(phone_number_string) == 4:
        ...
        if phone_number_string[3] == "-":
            ...
            phone_number_string = (
                phone_number_string[:3] + "-" + phone_number_string[3:]
            )
    ...
    if len(phone_number_string) == 8:
        ...
        # this is for going forwards
            ...
            phone_number_string = (
                phone_number_string[:7] + "-" + phone_number_string[7:]
            )
            return phone_number_string
