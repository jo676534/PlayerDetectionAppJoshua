...
video_trimmer_card = dbc.Card(
    id="trimmer_card",
    children=[
        dcc.Store(id="blacklist", storage_type="memory"),
        dbc.CardHeader(
            children=[
                html.H4(
                    "Video Trimmer",
                ...
                html.Div(
                    dcc.Loading(
                        dbc.Button(
                            id="run_scene_segmentation_button",
                            children="Run Scene Segmentation!",
                            color="success",
                        )
                    ),
        ...
        dbc.ButtonGroup(
            [
                ...
                dbc.Button(
                    "Save and Quit",
                    id="save-and-quit",
                    className="save-and-quit",
                    href="/apps/home",
                    size="md",
                ),
                # dbc.Button('Save and Continue',
                #            id='save-and-continue', href='/apps/dashboard', size="lg"),
                dbc.Button("Save and Continue", id="save-and-continue", size="md"),
            ],
            style={"width": "100%"},
        ),
        # dbc.Modal(
...
@app.callback(Output("frame_display_VE", "children"), Input("frame_VE", "data"))
...
def update_frame(
    ...
    if cbcontext == "previous_VE.n_clicks":
        ...
        data = (
            start
            if (start != None and start >= slider_min and start <= slider_max)
            else data
        )
        return data, data
    elif cbcontext == "jumpEnd.n_clicks":
        data = (
            end if (end != None and end >= slider_min and end <= slider_max) else data
        )
        return data, data
...
@app.callback(
    Output("trimmer-output-containerVE", "children"), Input("frame-trimmer", "value")
)
...
def display_dropdown(n_clicks, _, children, start, end):
    ...
    if "index" in input_id:
        ...
        children = [
            trim for trim in children if "'index': " + str(delete_trim) not in str(trim)
        ]
    else:
        if start != None and end != None and end > start:
            new_trim = html.Div(
                dbc.Row(
                    ...
                    children=[
                        html.Div(
                            f"Trimming Frame {start} through Frame {end}",
                            className="current_frame",
                        ),
                        dcc.Store(
                            id={"type": "trim-start", "index": n_clicks},
                            storage_type="memory",  # change to session when in prod?
                            data=start,
                        ),
                        dcc.Store(
                            id={"type": "trim-end", "index": n_clicks},
                            storage_type="memory",  # change to session when in prod?
                            data=end,
                        ),
                        dbc.Button(
                            "Remove",
                            id={"type": "remove-trim", "index": n_clicks},
                            color="danger",
                        ),
                    ],
...
@app.callback(
    Output("run_scene_segmentation_button", "n_clicks"),
...
def call_scene_segmentation(n_clicks):
    if n_clicks is None:
        ...
        res = scene_segmentation_api.begin_scene_segmentation(
            temp_variables_name, temp_variables_key
        )
