...
first_name = html.Div(dbc.Input(id="user_first_name", placeholder="First Name"))

last_name = html.Div(dbc.Input(id="user_last_name", placeholder="Last Name"))
...
sign_up_button = html.Div(
    [
        dbc.Button(
            id="sign_up_button_id_sports_science_ai",
            children="Sign Up",
            color="primary",
            disabled=True,
        ),
...
card = dbc.Card(
    [
        ...
        dbc.CardFooter(
            id="card_footer_id", children=sign_up_button, style={"textAlign": "center"}
        ),
    ]
)
...
after_request_to_sign_up = html.Div(
    html.H5(
        "Your request has been made and the admin has been notified to create your account. \
                            Please check your email for a temporary password and login. Your temporary password will expire in 7 days."
    )
)

button_return_login = html.Div(
    dbc.Button(
        id="return_login_button_id",
...
@app.callback(
    Output("card_footer_id", "children"),
...
def ask_admin_to_sign_up(
    n_clicks, user_first_name, user_last_name, user_email, phone_number
):
    ...
    if n_clicks is None:
        ...
        aws_email.send_email_to_admin(
            user_first_name, user_last_name, user_email, phone_number
        )
