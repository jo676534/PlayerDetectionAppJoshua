    ...
    children=[
                ...
                dbc.Modal(  # modal for searching for players
                    [
                        ...
                        dbc.ModalBody(
                            [
                                html.Div(
                                    "Player Name:", style={"display": "inline-block"}
                                ),
                                dbc.Input(
                        ...
                        dbc.ModalFooter(
                            [
                                dbc.Button(
                                    "Close Modal", id="player_close_modal", n_clicks=0
                                )
                            ]
                        ),
                    ],
        ...
        dbc.CardBody(  # the body includes the
            [
                ...
                html.Br(),
                html.Div(
                    "Player ID to Remove from Team A:",
                    style={"display": "inline-block"},
                ),
                dbc.Input(
                    id="player_input_remove_a",
                    placeholder="Player ID",
                    type="number",
                    min=0,
                    step=1,
                ...
                html.Br(),
                html.Div(
                    "Player ID to Remove from Team B:",
                    style={"display": "inline-block"},
                ),
                dbc.Input(
                    id="player_input_remove_b",
                    placeholder="Player ID",
                    type="number",
                    min=0,
                    step=1,
        ...
        dbc.CardFooter(
            [
                ...
                html.Div(
                    "Submit Button when finished. Needs to check for at least 11 players on both teams, Team ids filled out, date filled out, anything else?"
                ),
                html.Div(
                    id="hidden_div_set_player"
                ),  # used to update other parts like: 1. close the modal, 2. clear the modal player name input
                html.Div(
                    id="hidden_div_add_player"
                ),  # used to update other parts like: 1. clear the player id input
                html.Div(
                    id="hidden_div_remove_player"
                ),  # used to update other parts like: 1. regen the table with the removed player
...
team_upload_card = dbc.Card(
    id="team_upload_card",
    children=[
        dbc.CardHeader([html.H2("Team Info Card")]),
        dbc.CardBody(
            [
                ...
                dbc.Modal(  # modal for searching for teams
                    [
                        ...
                        dbc.ModalBody(
                            [
                                html.Div(
                                    "Team Name:", style={"display": "inline-block"}
                                ),
                                dbc.Input(
                                    id="team_name",
                        ...
                        dbc.ModalFooter(
                            [
                                dbc.Button(
                                    "Close Modal", id="team_close_modal", n_clicks=0
                                )
                            ]
                        ),
                    ],
...
def search_team(search_clicks, add_clicks, confirm, deny, team_name):
    ...
    if cbcontext == "team_search.n_clicks" and search_clicks:
        ...
        if len(df_out) > 1:
            return (
                f"This query returned {len(df_out)} teams. Please be more specific.",
                {"display": "none"},
                {"display": "none"},
            )
        elif len(df_out) == 0:
            return (
                "No teams were found with that name.",
                {"display": "none"},
                {"display": "none"},
            )
        else:
            for index, df in df_out.iterrows():
                ...
                t_id = int(id)
                return (
                    f"Team ID: {id} // Team Name: {t_name}",
                    {"display": "none"},
                    {"display": "block"},
                )  # displays the add to team a or team b buttons
    ...
    # part one for adding a new team
        ...
        return (
            f'This will create a new team named "{t_name}". Are you sure?',
            {"display": "block"},
            {"display": "none"},
        )  # displays the confirm/deny buttons used in the next part of the callback
...
def toggle_player_modal(open, close, hidden_div, is_open):
    cbcontext = [p["prop_id"] for p in dash.callback_context.triggered][0]
    if (
        cbcontext == "player_open_modal.n_clicks"
        or cbcontext == "player_close_modal.n_clicks"
        or (cbcontext == "hidden_div_set_player.children" and hidden_div == "close")
    ):
        return not is_open
    return is_open
...
def search_player(search_clicks, hidden_div, player_name):
    ...
    if cbcontext == "player_search.n_clicks" and search_clicks:
        ...
        if len(df_out) > 1:
            return (
                f"This query returned {len(df_out)} players. Please be more specific.",
                {"display": "none"},
                {"display": "none"},
                player_name,
            )
            ...
            return (
                "No players were found with that name.",
                {"display": "none"},
                {"display": "none"},
                player_name,
            )
            ...
            for index, df in df_out.iterrows():
                ...
                p_id = int(id)
                return (
                    f"Player ID: {id} // Player Name: {p_name}",
                    {"display": "none"},
                    {"display": "block"},
                    player_name,
                )
...
@app.callback(
    ...
    Output(
        "hidden_div_set_player", "children"
    ),  # used to update other parts like: 1. close the modal, 2. clear the input
...
def add_player_to_team(add_a, add_b, hidden_div, player_id):
    ...
    # add player to team
    if (
        cbcontext == "add_player_a.n_clicks"
        and add_a is not None
        ...
        if df_player_a is None:  # if so we create the df with their info
            for index, df in df_out.iterrows():
                ...
                df_player_a = pd.DataFrame(
                    data, columns=["player_id", "name", "initials", "team_id", "jersey"]
                )
        ...
        if df_player_a is None:
            output_a = None
        else:
            output_a = df_player_a.to_dict("records")

        if df_player_b is None:
            output_b = None
        else:
            output_b = df_player_b.to_dict("records")

        return output_a, output_b, output_text, None

    elif (
        cbcontext == "add_player_b.n_clicks"
        and add_b is not None
        and player_id is not None
    ):
        # get df with the player's info
        sql = f"""SELECT * FROM df_player WHERE player_id={player_id}"""
        ...
        if player_id is None and (add_a or add_b):
            output_text = "Need to input a Player ID first."
        else:
            output_text = None
