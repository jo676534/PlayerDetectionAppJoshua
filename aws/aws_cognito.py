...
def login_coach_user(user_email, user_password):
    ...
    if user_email == None or user_password == None:
        ...
        try:
            ...
            print(
                "User access token that needs to be saved in database: ",
                get_token_for_current_user,
            )

            user_signed_in_response = cognito_client_instance.get_user(
                AccessToken=str(get_token_for_current_user)
            )

            checks_if_user_signed_in_metadata = user_signed_in_response[
                "ResponseMetadata"
            ]

            checks_if_user_signed_in = checks_if_user_signed_in_metadata[
                "HTTPStatusCode"
            ]
...
def attempt_to_sign_up_coach_user(
    user_firstname, user_lastname, user_email, user_password, user_phone_number
):

    if (
        user_firstname == None
        or user_lastname == None
        or user_email == None
        ...
        formated_phone = (
            "+1"
            + temp_phone_number[:3]
            + (temp_phone_number[4:7] + temp_phone_number[8:])
        )
        try:
...
def ask_admin_to_sign_user_up(user_firstname, user_lastname, user_email, phone_number):
    if (
        ...
        aws_email.send_email_to_admin(
            user_firstname, user_lastname, user_email, phone_number
        )
