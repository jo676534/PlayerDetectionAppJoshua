...
def add_editable_box(
    ...
    fig.add_annotation(  # ((x0+x1)/2)
        ...
        font=dict(
            family="Roboto", size=13, color="#ffffff"  # "Courier New, monospace",
        ),
        align="center",
        arrowhead=2,
        arrowsize=1,
...
track_card = dbc.Card(
    ...
    children=[
        ...
        dbc.CardFooter(
            [
                html.H6(
                    "Add Track & Delete Section",
                    className="add_track_section_header",
                    style={"margin-bottom": "5px"},
                ),
                html.Div(
                    [
                        dbc.Input(
                            id="dashboard_input_start",
                            placeholder="Start",
                            type="number",
                            min=0,
                            step=1,
                    ...
                    children=[
                        ...
                        dbc.Col(
                            [
                                dbc.RadioItems(
                                    options=[
                                        {"label": "Select a Team View", "value": str(1)}
                                    ],
                                    id="radio_players_A",
                                    className="radio_items",
                                )
...
def manual_annotation(
    graph_relayout, frame, player_id, game_id, slider_min, slider_max
):
    ...
    if new_num_boxes < old_num_boxes:
        ...
        if (
            len(df) != 1
        ):  # if len(df) is 1 then the graph length is zero and graph relayout will have no values to match with
        ...
        df_detections = api_detections.get_detection_data(
            game_id, slider_min, slider_max
        )  # regenerate data

        return (
            "Detection box deleted from frame {} and track {}".format(frame, track_id),
            None,
        )
    ...
    # ADD BOX ------------------------------------
        ...
        if player_id is None:
            ...
            if not err:
                ...
                track_id = -2 - int(
                    player_id
                )  # the smallest player_id is 0 and every player has a unique id, therefore manual track annotation ids can be consistently assigned through this simple formula
                initials = api_detections.get_player_initials(player_id)

                api_detections.add_detection(
                    game_id, frame, x0, y0, x1, y1, track_id, player_id, initials
                )  # add box
                df_detections = api_detections.get_detection_data(
                    game_id, slider_min, slider_max
                )  # regenerate data
                ...
                return (
                    "This player already has a detection assigned to them in this frame",
                    None,
                )
...
def add_track_function(
    ...
    if (
        add_clicks is None and delete_clicks is None
    ):  # if this callback was accidently called or initialized
        return None, start_frame, final_frame, player_id
    if (start_frame is None) or (final_frame is None):
        return (
            "Must have inputs for start and final frame.",
            start_frame,
            final_frame,
            player_id,
        )
        ...
        return (
            "Start frame must be less than final frame.",
            start_frame,
            final_frame,
            player_id,
        )
    ...
    if cbcontext == "delete_section.n_clicks":
        if track_id is None:
            return (
                "Must have an intended track selected.",
                start_frame,
                final_frame,
                player_id,
            )

        api_detections.delete_detection_section(
            game_id, start_frame, final_frame, track_id
        )  # don't check and just purge the section even if there isn't anything there or only partially there within the single api call (simpler and doesn't matter much)
        df_detections = api_detections.get_detection_data(
            game_id, slider_min, slider_max
        )

        return (
            "Deleted selected dections from the track.",
            start_frame,
            final_frame,
            player_id,
        )
    # Add Track
        ...
        if player_id is None:
            return (
                "Must have an intended player selected.",
                start_frame,
                final_frame,
                player_id,
            )
                        ...
                        [
                            dbc.RadioItems(
                                options=[
                                    {
                                        "label": "("
                                        + str(a_row.iloc[i]["jersey"])
                                        + ") "
                                        + str(a_row.iloc[i]["name"]),
                                        "value": str(a_row.iloc[i]["player_id"]),
                                    }
                        ...
                        [
                            dbc.RadioItems(
                                options=[
                                    {
                                        "label": "("
                                        + str(b_row.iloc[i]["jersey"])
                                        + ") "
                                        + str(b_row.iloc[i]["name"]),
                                        "value": str(b_row.iloc[i]["player_id"]),
                                    }
                                    for i in range(0, len(b_row))
                                ],
                                id="radio_players_A",
                                className="radio_items",
                            )
...
@app.callback(
    Output("track_container", "children"),
...
def display_2(
                        ...
                        dbc.Col(
                            [
                                dbc.RadioItems(
                                    options=[
                                        {
                                            "label": "Track ID: " + str(all_tracks[i]),
                                            "value": str(all_tracks[i]),
                                        }
    ...
    if (
        cbcontext == "hidden_div_j1.children"
        or cbcontext == "radio_players_A.value"
        or cbcontext == "hidden_div_j2.children"
        or cbcontext == "switches-input.value"
    ):
        if (track_state == 0 or track_state == 1) and (
            cbcontext == "hidden_div_j2.children" or cbcontext == "switches-input.value"
        ):
            button_id = "all_tracks_bt"
        elif (track_state == 2) and (
            cbcontext == "hidden_div_j1.children"
            or cbcontext == "hidden_div_j2.children"
            or cbcontext == "switches-input.value"
        ):
            button_id = "viewable_tracks_bt"
        elif (track_state == 3) and (
            cbcontext == "radio_players_A.value"
            or cbcontext == "hidden_div_j2.children"
            or cbcontext == "switches-input.value"
        ):
                            ...
                            [
                                dbc.RadioItems(
                                    options=[
                                        {
                                            "label": "Track ID: "
                                            + str(viewable_tracks[i]),
                                            "value": str(viewable_tracks[i]),
                                        }
                                        for i in range(0, len(viewable_tracks))
                                    ],
                                    id="radio_all_tracks",
                                )
                            ],
                                            ...
                                            options=[
                                                {
                                                    "label": "Track ID: "
                                                    + str(player_tracks[i]),
                                                    "value": str(player_tracks[i]),
                                                }
                                                for i in range(len(player_tracks))
                                            ],
                                            id="radio_all_tracks",
                                        )
                                    ],
...
@app.callback(Output("frame_display_DB", "children"), Input("frame_DB", "data"))
...
def update_player_tracks(
    assignBt, unassignBt, track_id, player_id, game_id, slider_min, slider_max
):
    global df_detections
    cbcontext = [p["prop_id"] for p in dash.callback_context.triggered][0]
...
def draw_tracks(fig, currentFrame, switches_value, slider_min, slider_max, game_id):
    ...
    if unassinged_is_checked and assigned_is_checked:
        for i in range(len(frame_df)):
            ...
            add_editable_box(
                fig, track_id, player_id, initials, x0, y0, x1, y1, initials_is_checked
            )
...
def update_player(
    ...
    fig = px.imshow(
        get_frame(current_frame - 1), binary_backend="jpg"
    )  # should subtract 1 b/c the video's frames are zero indexed while the slider is 1 indexed
    fig.update_layout(
        xaxis={
...
def player_state(play_button, video_state, interval_state):
    ...
    text = html.Img(
        src=f"https://github.com/dianabisbe/Images/blob/main/{string}.png?raw=true",
        style={"height": "30px"},
    )
...
def update_frame(
    ...
    if cbcontext == "previous_DB.n_clicks":
        ...
        if track_id:
            ...
            raise PreventUpdate
    elif cbcontext == "interval_DB.n_intervals":
        data += 1
        return data, data
...
def initialize_section_and_slider(
    dropdown_value, prev, next, hidden_div, stored_section_value, game_id
):
    global sections
    global new_section
    cbcontext = [p["prop_id"] for p in dash.callback_context.triggered][0]
...
def initialize_globals(test, game_id):
    ...
    sectionOptions = [
        {"label": "Section {}".format(i + 1), "value": i} for i in range(sections)
    ]
...
@app.callback(
    ...
    Input(
        "dropdown_DB", "value"
    ),  # whenever the dropdown is updated we want to trigger this input to regenerate the data
    State("section_DB", "data"),
    State("game_id", "data"),
)
def initializer(dropdown_value, stored_section_value, game_id):
    ...
    div = html.Div(
        children=[
            dbc.Button(
                str(a_name), id="but7", outline=True, style={"font-size": "12px"}
            ),
            dbc.Button(
                str(b_name),
                id="but8",
