...
email_component = html.Div(
    [dbc.Input(type="email", id="user_email", placeholder="Enter email"), html.Br()],
...
coach_sign_up_component = html.Div(
    dcc.Link(
        children="Sign Up as a Coach User?",
        id="coach_sign_up",
        href="/apps/sign_up_coach",
        refresh=True,
    )
)
...
sportscience_member_sign_up_component = html.Div(
    dcc.Link(
        children="Sign Up as a Sports Science AI User?",
        id="sportscience_member_sign_up",
        href="/apps/sign_up_sportscience",
        refresh=True,
    )
)
...
forgot_password_button = html.Div(
    # dbc.Button(id='forgot_password_button',children='Forgot Password?', color='danger', className='me-1',href='/apps/forgot_password'),
...
sign_in_button_coach = html.Div(
    dbc.Button(
        id="sign_in_button_coach_id",
        children="Sign In as a Coach User?",
        color="primary",
    ),
...
sign_in_button_sportscience = html.Div(
    dbc.Button(
        id="sign_in_button_sportscience_id",
        children="Sign In as a Sports Science AI User",
    ),
    style={"padding-bottom": "5px"},
)


about_us_hyperlink = html.Div(
    dcc.Link(children="About Us", id="about_us", href="/apps/about_us", refresh=True)
)
...
email_hyperlink_component = dcc.Link(
    children="sportscienceai@gmail.com", href="google.com"
)
...
incorrect_password_alert_1 = html.Div(
    ...
    children=[
        dbc.Alert(
            id="incorrect_password_alert_id_1",
            children=[
...
incorrect_password_alert_2 = html.Div(
    ...
    children=[
        dbc.Alert(
            id="incorrect_password_alert_id_2",
            children=[
                html.Div("Invalid Email or Password!"),
                dcc.Link(
                    children="Reset Password?",
    ...
    children=[
        ...
        dbc.CardBody(
            [
                ...
                password_component,
                html.Div(
                    id="incorrect_password_div", children=None, style={"color": "red"}
                ),
...
def attempt_to_sign_in_coach_user(
    n_clicks,
    user_email,
    user_password,
    is_open,
    ...
    if n_clicks is None:
        ...
        res = aws_cognito.login_coach_user(
...
def attempt_to_sign_in_sportscience_ai_user(
    n_clicks, user_email, user_password, alert_is_open
):

    if n_clicks is None:
        raise PreventUpdate
    else:
        res = aws_cognito.login_sportscience_ai(
            user_email=user_email, password=user_password
        )

        if res == "NEW_PASSWORD_REQUIRED":
            if (
                aws_cognito.admin_sets_user_password(user_email, user_password)
                == "success"
            ):
                alert_is_open = False
