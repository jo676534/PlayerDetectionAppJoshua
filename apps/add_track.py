...
right_side = dbc.Card(
    ...
    children=[
        dbc.CardHeader(
            [html.H2("Bounding Box Submit Area"), html.Div(id="which_player")]
        ),
        dbc.CardBody(
            [
                ...
                html.Div(
                    "1. The video player is locked at the start frame until the tracker is finished running."
                ),
                html.Div(
                    "2. Click and drag to draw an annotation bounding box around the intented player on the image."
                ),
                html.Div(
                    "3. An already existing box can be interacted with by clicking on it's edges."
                ),
                html.Div(
                    "  3a. It can then be modified by draging one of the corners."
                ),
                html.Div(
                    '  3b. It can then be deleted by clicking "Erase active shape" in the top right corner above the image.'
                ),
                html.Div(
                    "4. Ensure there is only one bounding box around the intended player."
                ),
                html.Div(
                    '5. Click "Start Tracker" below and wait for it to finish running.'
                ),
        ...
        dbc.CardFooter(
            [
                ...
                html.Div(
                    "There are now three primary options for using the detection track:"
                ),
                html.Div(
                    '1. If there are no errors with the detections you can click "Save Detection Track" to finish.'
                ),
                html.Div(
                    "2. If part of the track is correct you can input a start and end frame below before saving the detection track to only save the frames within that window."
                ),
                html.Div(
                    "3. If you'd like to remove the detections all together and start over you can reset the tracker and try again."
                ),
                html.Div(""),
                # html.Br(),
                html.Div(
    ...
    children=[
        ...
        dbc.CardBody(
            [
                ...
                html.Div(
                    children=[  # needs to be properly initialized //////////////////////////////////////////////////////////////////////
                        dcc.Interval(
                            id="interval_add",
                            disabled=False,
...
def submit_box(n_clicks, graph_relayout, start_frame, final_frame):
    ...
    if not "shapes" in graph_relayout:
        ...
        if "shapes[0].x0" in graph_relayout:
            state = 99
            detections_df = demo.rt_track(
                start_frame,
                final_frame,
                graph_relayout["shapes[0].x0"],
                graph_relayout["shapes[0].y0"],
                graph_relayout["shapes[0].x1"],
                graph_relayout["shapes[0].y1"],
            )
            state = 1
        ...
        state = 99
        for box in graph_relayout[
            "shapes"
        ]:  # this will only have one iteration (b/c there should only be one bounding box)
            detections_df = demo.rt_track(
                start_frame, final_frame, box["x0"], box["y0"], box["x1"], box["y1"]
            )
        state = 1
        return "The tracking algorithm has now finished, you can review the output in the video player."
    else:
...
@app.callback(
    ...
    State(
        "input_start", "value"
    ),  # start_frame (represents the start value for a subset selection of frames)
...
def save_detection(
    ...
    if (start_input is None) and (final_input is None):
        ...
        api_detections.save_track(
            game_id, detections_df, start_frame, track_id, -1
        )  # Do NOT change -1. It NEEDS to be -1 for this
    ...
    # Good State: user input but and we need to use them
        ...
        detections_df = detections_df[
            start_input - start_frame : final_input - final_frame + 1
        ]
