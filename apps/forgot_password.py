...
password_description_component = html.Div([
                            ...
                            dbc.FormText(id='reset_length_description_id',children='* Your Password must include at least 8 characters'),
                            dbc.FormText(id='reset_uppercase_description_id',children='* At least one uppercase letter is needed'),
                            dbc.FormText(id='reset_lowercase_description_id',children='* At least one lowercase letter is needed'),
                            dbc.FormText(id='reset_number_description_id',children='* At least one number [0-9] is needed'),
                            dbc.FormText(id='reset_special_char_description_id',children="* At least one special character ['!','@','#','$', '%', '&','+','/'] is needed"),
]) 
...
email_component = html.Div(
        [
                dbc.Input(type='email',id='user_email_forgot_password',placeholder='Enter email'),
                html.Br()
...
drop_down = html.Div(children=dcc.Dropdown(id='type_of_user',options=[{'label':'Coach User','value':'cu'},{'label':'SportScience AI User','value':'spai'}],
                        style={'padding-left':'200px','padding-right':'200px'}))
...
an_error_occurred = html.Div(children= dbc.Alert(
...
form_to_reset_and_submit_component = html.Div(
                                        children=[
                                            ...
                                            dbc.Popover(id='popover_password',children='',trigger='hover',target='reset_password_input'),
                                            password_description_component,
                                            html.Br(),
                                            dbc.Input(id='confirm_password_input',type='password'),
                                            html.Br(),
                                            html.Div(children= dbc.Alert(
                                            ...
                                            html.Br(),
                                            html.Div(dbc.Button(id='submit_password_reset_button',children='Submit Password Reset',disabled=True),style={'textAlign':'center'}),
...
reset_password_module = html.Div(
    ...
    dbc.Modal(
        [
                ...
                dbc.ModalFooter(
                    html.H5('You will be redirected to the login page once successfully resetted password.')
                   # form_to_reset_and_submit_component
...
card_container = dbc.Card(
    ...
    children=
    [
        dbc.CardHeader(children=[html.Img(
                                        id='logo_login',
                                        src='https://github.com/dianabisbe/Images/blob/main/SportScienceLogo.png?raw=true')],
                                style={'background-color':'#000e44'}),
        dbc.CardBody(
            [   
                html.H4('Please enter your email and we will send a verifaction code to allow you to reset your password.'),
                html.Br(),
                email_component,
...
def validate_password_strength(user_password):
    ...
    if user_password is None:
        ...
        if list_of_styles[5] == True:
            return list_of_styles[0],list_of_styles[1],list_of_styles[2],list_of_styles[3],list_of_styles[4]
            
        else: return list_of_styles[0],list_of_styles[1],list_of_styles[2],list_of_styles[3],list_of_styles[4]


##########################################################################################


@app.callback(Output('submit_password_reset_button','disabled'),
...
@app.callback(Output('invalid_code_alert_reset_inside','children'),
...
def submit_password_reset(n_clicks,user_type,username,new_password,code_entered,is_open_error):
    if n_clicks is None:
        raise PreventUpdate
    
    else:
        res = aws_cognito.confirm_reset_any_user_password(user_type,username,code_entered,new_password)
...
@app.callback(Output('reset_password_module','is_open'),
...
@app.callback(Output('send_verifaction_code_reset','disabled'),
...
def sets_send_code_button(user_email,type_of_user):
    ...
    if user_email is None or type_of_user is None:
        ...
        else: return True,'Invalid Email!'
...
@app.callback(Output('send_verifaction_code_reset','children'),
...
def send_code_to_user_email(n_clicks,user_email,type_of_user,alert_is_open):
    ...
    if n_clicks is None:
        ...
        res = aws_cognito.reset_any_user_password(user_type=type_of_user,username=user_email)
        # print(res)
        #res = 'success'
        if res == 'success':
            alert_is_open = False
            ...
            error = res.split(':')[0]
            res = error
            return 'An error occurred! Try Again?',None,res,alert_is_open,